...
IMPLEMENT_GLOBAL_SHADER(FCopyConvergedLightmapTilesCS, "/Plugin/WirelessSignal/Private/LightmapBufferClear.usf", "CopyConvergedLightmapTilesCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FUploadConvergedLightmapTilesCS, "/Plugin/WirelessSignal/Private/LightmapBufferClear.usf", "UploadConvergedLightmapTilesCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FSelectiveLightmapOutputCS, "/Plugin/WirelessSignal/Private/LightmapOutput.usf", "SelectiveLightmapOutputCS", SF_Compute);
IMPLEMENT_GLOBAL_SHADER(FMultiTileClearCS, "/Plugin/WirelessSignal/Private/LightmapBufferClear.usf", "MultiTileClearCS", SF_Compute);
	...
	, LightmapTilePoolGPU(FIntPoint(Scene->Settings->LightmapTilePoolSize))
{
	...
	bDenoiseDuringInteractiveBake = Scene->Settings->DenoisingOptions == EWirelessSignalDenoisingOptions::DuringInteractivePreview;
	bOnlyBakeWhatYouSee = Scene->Settings->Mode == EWirelessSignalMode::BakeWhatYouSee;
...
void FSceneRenderState::SetupRayTracingScene(int32 LODIndex)
{
...

	{
		...
		SCOPED_DRAW_EVENTF(RHICmdList, WirelessSignalUpdateRayTracingScene, TEXT("WirelessSignal UpdateRayTracingScene %d Instances"), StaticMeshInstanceRenderStates.Elements.Num());
...
void FLightmapRenderer::Finalize(FRHICommandListImmediate& RHICmdList)
{
	...
	// Upload & copy converged tiles directly
	{
		...
		{
			SCOPED_DRAW_EVENTF(RHICmdList, WirelessSignalUploadConvergedTiles, TEXT("WirelessSignal UploadConvergedTiles %d tiles"), TileUploadRequests.Num());
			...
			if (!UploadTilePoolGPU.IsValid() || UploadTilePoolGPU->SizeInTiles.X < NewSize)
			{
				...
				UE_LOG(LogWirelessSignal, Log, TEXT("Resizing WirelessSignal upload tile pool to (%d, %d) %dx%d"), NewSize, NewSize, NewSize * GPreviewLightmapPhysicalTileSize, NewSize * GPreviewLightmapPhysicalTileSize);
			}
			...
			if (OutputRenderTargets[0] != nullptr)
			{
				...
				{
					FRDGTextureRef RenderTargetTileAtlas0 = GraphBuilder.RegisterExternalTexture(OutputRenderTargets[0], TEXT("WirelessSignalRenderTargetTileAtlas0"));
			...
			if (OutputRenderTargets[1] != nullptr)
			{
				...
				{
					FRDGTextureRef RenderTargetTileAtlas1 = GraphBuilder.RegisterExternalTexture(OutputRenderTargets[1], TEXT("WirelessSignalRenderTargetTileAtlas1"));
			...
			if (OutputRenderTargets[2] != nullptr)
			{
				...
				{
					FRDGTextureRef RenderTargetTileAtlas2 = GraphBuilder.RegisterExternalTexture(OutputRenderTargets[2], TEXT("WirelessSignalRenderTargetTileAtlas1"));
...

		{
			...
			while (!bScratchAllocationSucceeded)
			{
				...
				if (!bScratchAllocationSucceeded)
				{
					...
					UE_LOG(LogWirelessSignal, Log, TEXT("Resizing WirelessSignal scratch tile pool to (%d, %d) %dx%d"), NewSize, NewSize, NewSize * GPreviewLightmapPhysicalTileSize, NewSize * GPreviewLightmapPhysicalTileSize);
				}
			}
		}
	...
	// Output from working set to VT layers
	{
...

		{
			...
			{
				FRDGTextureRef RenderTargetTileAtlas = GraphBuilder.RegisterExternalTexture(OutputRenderTargets[0] != nullptr ? OutputRenderTargets[0] : OutputRenderTargets[1], TEXT("WirelessSignalRenderTargetTileAtlas0"));
			...
			if (OutputRenderTargets[2] != nullptr)
			{
				FRDGTextureRef RenderTargetTileAtlas = GraphBuilder.RegisterExternalTexture(OutputRenderTargets[2], TEXT("WirelessSignalRenderTargetTileAtlas2"));
...
void FLightmapRenderer::BackgroundTick()
{
	...
	if (bIsViewportNonRealtime && !bWasRunningAtFullSpeed)
	{
		...
		UE_LOG(LogWirelessSignal, Log, TEXT("WirelessSignal is now running at full speed"));
	}

	if (!bIsViewportNonRealtime && bWasRunningAtFullSpeed)
	{
		bWasRunningAtFullSpeed = false;
		UE_LOG(LogWirelessSignal, Log, TEXT("WirelessSignal is now throttled for realtime preview"));
	}
	...
	if (!bOnlyBakeWhatYouSee)
	{
		...
		if (Mip0WorkDoneLastFrame < NumWorkPerFrame)
		{
			...
			if (bIsViewportNonRealtime && FrameNumber % 100 == 0)
			{
				...
				UE_LOG(LogWirelessSignal, Log, TEXT("Working on: %s"), *AllNames);
			}
		}
...
void FLightmapRenderer::DeduplicateRecordedTileRequests()
{
	...
	UE_LOG(LogWirelessSignal, Log, TEXT("Tile deduplication removed %d tiles"), OldNum - RecordedTileRequests.Num());
}
