...
void FScene::GatherImportanceVolumes()
{
	...
	for (TObjectIterator<ALightmassImportanceVolume> It; It; ++It)
	{
		...
		if (WirelessSignal->World->ContainsActor(LMIVolume) && !LMIVolume->IsPendingKill())
		{
	...
	if (CombinedImportanceVolume.GetExtent().SizeSquared() == 0)
	{
		...
		else
		{
			...
			UE_LOG(LogWirelessSignal, Warning, TEXT("No importance volume found, so the scene bounding box was used.  You can optimize your scene's quality and lighting build times by adding importance volumes."));
...
void FScene::AddLight(LightComponentType* PointLightComponent)
{
	...
	if (bCastStationaryShadows)
	{
		...
		{
			UE_LOG(LogWirelessSignal, Log, TEXT("Ignoring light with ShadowMapChannel == -1 (probably in the middle of SpawnActor)"));
			return;
		}
	}
...
void FScene::AddGeometryInstanceFromComponent(UInstancedStaticMeshComponent* InComponent)
{
	...
	{
		UE_LOG(LogWirelessSignal, Log, TEXT("Skipping empty instanced static mesh"));
		return;
	}
...
void FScene::AddGeometryInstanceFromComponent(ULandscapeComponent* InComponent)
{
	...
	{
		UE_LOG(LogWirelessSignal, Log, TEXT("Skipping landscape with empty info object"));
		return;
	}
	...
	InstanceRenderState.SharedBuffersKey |= 1 << 29; // Use this bit to indicate it is WirelessSignal specific buffer (which only has FixedGridVertexFactory created)
...
void FScene::BackgroundTick()
{
	...
	else
	{
		...
		WirelessSignal->World->GetSubsystem<UWirelessSignalSubsystem>()->OnLightBuildEnded().Broadcast();
	}
}
...
void FScene::ApplyFinishedLightmapsToWorld()
{
	...
	UWorld* World = WirelessSignal->World;
...

		{
			...
			for (int32 InstanceIndex = 0; InstanceIndex < StaticMeshInstances.Elements.Num(); InstanceIndex++)
			{
				for (int32 LODIndex = 0; LODIndex < StaticMeshInstances.Elements[InstanceIndex].LODLightmaps.Num(); LODIndex++)
				{
					if (StaticMeshInstances.Elements[InstanceIndex].LODLightmaps[LODIndex].IsValid())
					{
						if (Settings->DenoisingOptions == EWirelessSignalDenoisingOptions::OnCompletion)
						{
...
#if 0
						{
...

							{
								FFileHelper::SaveStringToFile(
									...
									*FString::Printf(TEXT("%s_Irradiance_%dspp.pfm"), *Lightmap.Name, GWirelessSignalSamplesPerTexel),
									FFileHelper::EEncodingOptions::ForceAnsi
								);

								TArray<uint8> Bytes((uint8*)Color.GetData(), Color.GetTypeSize() * Color.Num());

								FFileHelper::SaveArrayToFile(Bytes, *FString::Printf(TEXT("%s_Irradiance_%dspp.pfm"), *Lightmap.Name, GWirelessSignalSamplesPerTexel), &IFileManager::Get(), EFileWrite::FILEWRITE_Append);
							}

							{
								FFileHelper::SaveStringToFile(
									FString::Printf(TEXT("PF\n%d %d\n-1.0\n"), Lightmap.GetSize().X, Lightmap.GetSize().Y),
									*FString::Printf(TEXT("%s_SH_%dspp.pfm"), *Lightmap.Name, GWirelessSignalSamplesPerTexel),
									FFileHelper::EEncodingOptions::ForceAnsi
								);

								TArray<uint8> Bytes((uint8*)SH.GetData(), SH.GetTypeSize() * SH.Num());

								FFileHelper::SaveArrayToFile(Bytes, *FString::Printf(TEXT("%s_SH_%dspp.pfm"), *Lightmap.Name, GWirelessSignalSamplesPerTexel), &IFileManager::Get(), EFileWrite::FILEWRITE_Append);
							}
						}
#endif
